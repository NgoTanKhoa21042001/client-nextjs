...
Khi cần thêm Css cho cả app: Ví dụ các thẻ cơ bản `body, html, a, p, h1, h2, h3, h4, h5, h6, ...` hay `*`, hoặc đôi khi thêm 1 số class để dùng toàn app thì cũng thêm ở đây

- Css ở file `src/app/globals.css`
- Nếu dùng tailwind thì nên dùng `@layer` để đảm bảo về tính dễ đọc cũng như độ ưu tiên css khi build
